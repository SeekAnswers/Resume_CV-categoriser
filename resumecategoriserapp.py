...
# Function to categorise resumes
def categorise_resumes(uploaded_files, output_directory):
...
st.title('Seek Answers Resume Categoriser')
st.subheader('Easily categorise those resumes/CVs')

# File uploader for PDF files
uploaded_files = st.file_uploader('Choose PDF files', type='pdf', accept_multiple_files=True)

# Text input for output directory
output_directory = st.text_input('Output directory', 'categorised_resumes')

# Button to trigger resume categorisation
if st.button('Categorise Resumes'):
    if uploaded_files and output_directory:
        results_df = categorise_resumes(uploaded_files, output_directory)
        st.write(results_df)
        results_csv = results_df.to_csv(index=False).encode('utf-8')
        st.download_button(
            label='Download results as CSV',
            data=results_csv,
            file_name='categorised_resumes.csv',
            mime='text/csv'
        )
        st.success('Resume categorisation and processing completed.')
    else:
